...
import eu.trentorise.smartcampus.vas.ifame.controllers.MenuController;
...
public class GestoreMenu {
	...
	// /**
	// * prende dal primo giorno del mese fino all ultimo tutti i piatti e crea
	// il
	...
	//
	// ArrayList<MenuDellaSettimana> mdsList = new
	// ArrayList<MenuDellaSettimana>();
	//
	...
	private static void updatePiatti(PiattoRepository piattoRepository,
				...
				for (Piatto piattodb : piattiPresentiNelDb) {
					if (piattodb.getPiatto_nome().compareTo(
						...
						for (Giudizio g : giudizidb) {
							if (g.getPiatto_id() == piattodb.getPiatto_id()) {
